...
def distill_train(all_logits, dur, epoch, model, optimizer, conf, G, labels_init, labels_one_hot, labels, idx_no_train, idx_train,
    ...
    if epoch>20 :
        ...
        for pvt_name, pt in t_model.named_parameters():#t1,t2
            pt.grad = torch.autograd.grad(t_loss, pt,create_graph=True)[0] #求model中的参数对tmodel中参数的梯度
            t_model_dict[pvt_name] = pt - my_t_lr * pt.grad #更新tmodel参数
