...
class GCN(nn.Module):
    ...
    def forward(self, features, model_dict=None):
        h = features
        for i, layer in enumerate(self.layers):
            if i != 0:
                h = self.dropout(h)
            if model_dict is not None:
                h = layer(self.g, h, weight=model_dict['layers.'+str(i)+'.weight'], bias=model_dict['layers.'+str(i)+'.bias'])
            else:
                h = layer(self.g, h)
...
class ogb_GCN(nn.Module):
    def __init__(self, g, in_feats, n_hidden, n_classes, n_layers, activation, dropout, use_linear=False):
        ...
        for i in range(n_layers):
            ...
            self.convs.append(
                GraphConv(in_hidden, out_hidden, "both", bias=bias))
            if use_linear:
                self.linear.append(
...
class GraphConv(nn.Module):
    ...
    def forward(self, graph, feat, weight=None, edge_weight=None, bias=None):
        r"""
